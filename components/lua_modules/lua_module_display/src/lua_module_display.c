...
static void lua_display_runtime_cleanup(void)
{
    ...
    /* Auto-present in case the script was force-stopped mid-frame */
...
static int lua_display_deinit(lua_State *L)
{
    ...
    /* Auto-present: if the script drew into a frame but forgot to call
     * present(), flush it now so the drawing reaches the LCD before the
     * HAL framebuffers are freed. */
    if (display_hal_is_frame_active()) {
        display_hal_present();
        display_hal_end_frame();
    }
