...
#endif
#if CONFIG_APP_CLAW_CAP_SERVO
#include "cap_servo.h"
#endif
#if CONFIG_APP_CLAW_CAP_EMOTION
...
#include "esp_check.h"
#include "esp_log.h"
#if CONFIG_APP_CLAW_CAP_ASR || CONFIG_APP_CLAW_CAP_VOICE
...
#if CONFIG_APP_CLAW_CAP_ASR
static esp_err_t app_cap_prepare_asr(const app_claw_config_t *config,
                                     const app_claw_storage_paths_t *paths)
{
    (void)config;
    (void)paths;

    /* ASR shares the same voice server as TTS */
    voice_ws_set_server(CONFIG_APP_CLAW_VOICE_SERVER_IP,
                        (uint16_t)CONFIG_APP_CLAW_VOICE_SERVER_PORT);
    app_asr_button_init();
    return ESP_OK;
}

static esp_err_t app_cap_register_asr(const app_claw_config_t *config,
                                      const app_claw_storage_paths_t *paths)
{
    (void)config;
    (void)paths;
    return cap_asr_register_group();
}
#endif

#if CONFIG_APP_CLAW_CAP_SERVO
static esp_err_t app_cap_register_servo(const app_claw_config_t *config,
                                        const app_claw_storage_paths_t *paths)
{
    (void)config;
    (void)paths;
    return cap_servo_register_group();
}
#endif

#if CONFIG_APP_CLAW_CAP_EMOTION
static esp_err_t app_cap_register_emotion(const app_claw_config_t *config,
                                          const app_claw_storage_paths_t *paths)
{
    (void)config;
    (void)paths;
    return emotion_video_register_capabilities();
}
#endif
...
#endif
#if CONFIG_APP_CLAW_CAP_VOICE
    { "cap_voice", "Voice TTS", "Register voice TTS cap", false, app_cap_prepare_voice, app_cap_register_voice },
#endif
#if CONFIG_APP_CLAW_CAP_ASR
    { "cap_asr", "Voice ASR", "Register voice ASR cap", false, app_cap_prepare_asr, app_cap_register_asr },
#endif
#if CONFIG_APP_CLAW_CAP_SERVO
    { "cap_servo", "Servo", "Register servo cap", false, NULL, app_cap_register_servo },
#endif
#if CONFIG_APP_CLAW_CAP_EMOTION
    { "cap_emotion", "Emotion", "Register emotion cap", true, NULL, app_cap_register_emotion },
#endif
};
...
#endif
#if CONFIG_APP_CLAW_CAP_VOICE
    { "cap_voice", "Voice TTS", false },
#endif
#if CONFIG_APP_CLAW_CAP_ASR
    { "cap_asr", "Voice ASR", false },
#endif
#if CONFIG_APP_CLAW_CAP_SERVO
    { "cap_servo", "Servo", false },
#endif
#if CONFIG_APP_CLAW_CAP_EMOTION
    { "cap_emotion", "Emotion", true },
#endif
};
