...
static display_arbiter_state_t s_state = {
    .owner = DISPLAY_ARBITER_OWNER_VIDEO,
};
...
static const char *display_arbiter_owner_to_str(display_arbiter_owner_t owner)
{
    ...
    case DISPLAY_ARBITER_OWNER_LUA:
        return "lua";
    case DISPLAY_ARBITER_OWNER_VIDEO:
        return "video";
    default:
        return "unknown";
    }
...
esp_err_t display_arbiter_acquire(display_arbiter_owner_t owner)
{
    ...
    ESP_GOTO_ON_FALSE(owner == DISPLAY_ARBITER_OWNER_LUA || owner == DISPLAY_ARBITER_OWNER_VIDEO, ESP_ERR_INVALID_ARG,
    ...
    } else if (owner == DISPLAY_ARBITER_OWNER_VIDEO) {
        if (s_state.lua_depth > 0) {
            ESP_LOGD(TAG, "video acquire refused: Lua holds display (lua_depth=%" PRIu32 ")", s_state.lua_depth);
            ret = ESP_ERR_INVALID_STATE;
            goto fail;
        }
        if (s_state.owner != DISPLAY_ARBITER_OWNER_VIDEO) {
            ESP_GOTO_ON_ERROR(display_arbiter_change_owner_locked(DISPLAY_ARBITER_OWNER_VIDEO), fail, TAG,
                              "switch to video owner failed");
            notify_owner = DISPLAY_ARBITER_OWNER_VIDEO;
            owner_changed = true;
        }
    }
