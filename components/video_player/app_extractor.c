...
static esp_err_t init_audio_decoder(app_extractor_t *extractor)
{
    ...
    // Static flags to track registered decoders (optimized for fast switching)
    static bool aac_decoder_registered = false;
    static bool mp3_decoder_registered = false;
    ...
    case EXTRACTOR_AUDIO_FORMAT_AAC: {
        ...
        // Only register AAC decoder if not already registered (optimization for fast file switching)
        if (!aac_decoder_registered) {
            ret = esp_aac_dec_register();
            if (ret != ESP_OK) {
                ESP_LOGE(TAG, "Failed to register AAC decoder: %d", ret);
                return ret;
            }
            aac_decoder_registered = true;
        }
    ...
    case EXTRACTOR_AUDIO_FORMAT_MP3: {
        ...
        // Only register MP3 decoder if not already registered (optimization for fast file switching)
                             ...
                             app_extractor_handle_t *ret_extractor)
{
    ...
    ESP_LOGD(TAG, "App extractor initialized%s", audio_dev ? " with audio" : "");
...
esp_err_t app_extractor_deinit(app_extractor_handle_t handle)
{
    ...
    ESP_LOGD(TAG, "App extractor deinitialized");
    return ESP_OK;
}
