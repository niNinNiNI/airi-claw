...
typedef struct app_stream_adapter_t {
    ...
    bool eos_reached;                         /*!< End of stream reached flag */
                                  ...
                                  app_stream_adapter_handle_t *ret_adapter)
{
    ...
    ESP_LOGD(TAG, "Stream adapter initialized%s with event group control", config->audio_dev ? " with audio" : "");
    *ret_adapter = adapter;
    return ESP_OK;
}
...
esp_err_t app_stream_adapter_is_eos(app_stream_adapter_handle_t handle, bool *eos)
{
    if (handle == NULL || eos == NULL) {
        return ESP_ERR_INVALID_ARG;
    }

    app_stream_adapter_t *adapter = (app_stream_adapter_t *)handle;
    *eos = adapter->eos_reached;
...
esp_err_t app_stream_adapter_deinit(app_stream_adapter_handle_t handle)
{
    ...
    ESP_LOGD(TAG, "Stream adapter deinitialized");
    return ESP_OK;
}
