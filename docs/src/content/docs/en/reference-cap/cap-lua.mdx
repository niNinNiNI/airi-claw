...
- `path` supports two forms:
  - Short relative path: e.g. `hello.lua` or `demo/hello.lua` (auto-expanded to `${base_dir}/...` and must not contain `..`).
  - Skill read-only script absolute path: e.g. `/fatfs/skills/light_switch/scripts/led_strip_switch.lua` (normally produced by `{CUR_SKILL_DIR}` expansion, must stay under the Skill root, and must not contain `..`).
- `lua_list_scripts` recursively scans the Script Base Dir and returns relative paths.
- `lua_list_scripts.prefix` uses a relative-path prefix, and `keyword` does case-insensitive matching against relative paths.
