...
- `path` 必须是 `.lua` 文件。
- `path` 可用两种形式：
  - 简短相对路径：如 `hello.lua` 或 `demo/hello.lua`（会自动拼成 `${base_dir}/...`，且不能包含 `..`）。
  - Skill 只读脚本绝对路径：如 `/fatfs/skills/light_switch/scripts/led_strip_switch.lua`（通常由 `{CUR_SKILL_DIR}` 占位符展开得到，必须位于 Skill 根目录下，且不能包含 `..`）。
- `lua_list_scripts` 递归扫描 Script Base Dir，并返回相对路径列表。
- `lua_list_scripts.prefix` 使用相对路径前缀，`keyword` 支持对相对路径做大小写不敏感匹配。
